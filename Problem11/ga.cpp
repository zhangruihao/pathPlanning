...
class GA {
  private:
    ...
    double yellow_rate;    //经过所有黄色点的个体占种群总体的比例
    unordered_set<ULL> s;  //种群个体的hashCode的集合,用于去除重复个体
  public:
    vector<Node> pop;    //族群
    vector<Node> child;  //本次种群的后代
    GA(vector<Node> p, int pop_size, double cross_rate, double mutation_rate, double yellow_rate) {
        this->pop_size = pop_size;
        this->pop = p;
        this->cross_rate = cross_rate;
        this->mutation_rate = mutation_rate;
        this->yellow_rate = yellow_rate;
    }
    //非支配排序, 返回应保留个体在pop_select中的下标
    vector<int> NSGA_II(vector<Node> pop_select) {
        int lower[pop_select.size()];  // lower[i]表示第i个个体劣于其他个体的数量
        int rank[pop_select.size()];   // rank[i], 种群第i个个体所在支配集层数
        set<int> dominate[pop_select.size()];// dominata[i]表示pop[i]支配的个体
        for (int i = 0; i < pop_select.size(); i++) {
            dominate[i].clear();
            lower[i] = 0;
            for (int j = 0; j < pop_select.size(); j++)
                if (pop_select[i] < pop_select[j]) lower[i]++;
                else if (pop_select[j] < pop_select[i]) dominate[i].insert(j);
        }
        int rk = 1;
        queue<int> q;
        for (int i = 0; i < pop_select.size(); i++) {
        ...
        for (int i = 0; i < pop_select.size(); i++) mp[rank[i]].push_back(i);
        vector<int> survival;  //被保留的个体在pop中的下标
        vector<int> lastSeq; //最后一层非支配集合
        for (auto& it : mp) {
            if (survival.size() + it.second.size() <= int(pop_size * yellow_rate))
        ...
        if (lastSeq.size()) {
            ...
            for (int i = 0; i < optNum; i++) {
                ...
                for (int j = 0; j < lastSeq.size(); j++)
                    d[i].x = pop_select[lastSeq[i]].w[i], d[i].y = i;
            ...
            int numAdd = int(pop_size * yellow_rate) - survival.size();
    ...
    void select() {
        ...
        vector<Node> popHasYellow, popNoYellow; //经过必经点的种群和不经过毕竟点的种群
        vector<Node> new_pop;//新种群
        //将种群中经过所有必经点的, 和没有经过必经点的分离
        for (int i = 0; i < pop.size(); i++) {
            if (checkHasAllYellow(pop[i].path)) {
                popHasYellow.push_back(pop[i]);
            } else {
                popNoYellow.push_back(pop[i]);
            }
        }
        // cout << popHasYellow.size() << " " << popNoYellow.size() << endl;
        vector<int> survival = NSGA_II(popHasYellow);
        // cout << survival.size() << endl;
        for (int i = 0; i < survival.size(); i++)
            new_pop.push_back(popHasYellow[survival[i]]);
        vector<int> shuffle;
        for (int i = 0; i < popNoYellow.size(); i++)
            shuffle.push_back(i);
        random_shuffle(shuffle.begin(), shuffle.end(), myrandom);
        int add = min(popNoYellow.size(), pop_size - new_pop.size());
        for (int i = 0; i < add; i++)
            new_pop.push_back(popNoYellow[shuffle[i]]);
        pop = new_pop;
        // cout << pop.size() << endl;
    }
    //交叉配对
    ...
    void outputResult(int mode = 0) {
        ...
        for (int i = 0; i < pop.size(); i++) {
            if (lower[i]) continue;
            vector<PII> p;
            if(!checkPath(pop[i].path) && !checkHasAllYellow(pop[i].path)) {
                puts("Error");
            }
