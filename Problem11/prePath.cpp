...
int g[55][55];         // 用于存储原始输入01矩阵
double f[50][50][10];  // fk[i][j][k] 用于记录点(i, j)的第k个指标
...
void readData() {
    freopen("Problem_11.txt", "r", stdin);
    cin >> n >> m;
    int num = 0;
    for (int i = 1; i <= n; i++)
        for (int j = 1; j <= m; j++) {
    ...
    cin.clear();
    freopen("START_GOAL_11.txt", "r", stdin);
    cin >> start.y >> start.x;
    cin >> goal.y >> goal.x;
    cin.clear();
    freopen("Yellow_areas_11.txt", "r", stdin);
    cin >> t;
    yellow_points.resize(t);
    yellow_idx.resize(t);
    for (int i = 0; i < t; i++)
        cin >> yellow_points[i].y >> yellow_points[i].x;
}
...
void graph_init() {
    ...
    for (int i = 0; i < yellow_points.size(); i++) {
        for (int j = 0; j < keyPoint.size(); j++) {
            if (keyPoint[j] == yellow_points[i]) {
                yellow_idx[i] = j;
                break;
            }
        }
    ...
    // cout << "Yellow_idx: ";
    // for (int i = 0; i < yellow_idx.size(); i++) {
    //     cout << yellow_idx[i]<< endl;
    //     cout << keyPoint[yellow_idx[i]].y  << " " << keyPoint[yellow_idx[i]].x << endl;
    // }
    // cout << endl;
}
...
bool checkHasAllYellow(vector<int> &path) {
    for (int i = 0; i < yellow_idx.size(); i++)
        if (!count(path.begin(), path.end(), yellow_idx[i]))
            return false;
    return true;
}